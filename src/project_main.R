...
pie3D(pdata$atot,labels=pdata$Age_group, labelcex=0.9, explode=0.1,col=colr, main="Suicide and Age Groups ")
...
ca_type%>%ggplot(aes(x=Type,y=ctot,color=Gender))+geom_point()+scale_color_manual(values=colr)+theme(legend.position = "bottom",axis.text.x = element_text(angle=90,vjust=0.5))+labs(x="Causes",y="Count")
...
#Analysing the trend for the major causes for the suicides
...
options(repr.plot.width=6, repr.plot.height=4) 
sc1<-sc_data%>%filter(!State %in% c("Total (All India)","Total (States)","Total (Uts)"))%>% select(State,Year,Total) %>% group_by(State)%>% summarise(tot=sum(Total)) %>% arrange(desc(tot))%>%head(10)%>%  ggplot(aes(x=factor(State,level=State),y=tot,color=State))+geom_point(size=4)+geom_segment(aes(xend=State,y=0,yend=tot),size=2)+theme(legend.position="none",axis.text.x=element_text(angle=90))+scale_color_manual(values=colr)+geom_text(aes(label=tot),vjust=0.3)+labs(x="State - More Suicides")
sc2<-sc_data%>%filter(!State %in% c("Total (All India)","Total (States)","Total (Uts)"))%>% select(State,Year,Total) %>% group_by(State)%>% summarise(tot=sum(Total)) %>% arrange(desc(tot))%>%tail(10)%>%   ggplot(aes(x=factor(State,level=State),y=tot,color=State))+geom_point(size=4)+geom_segment(aes(xend=State,y=0,yend=tot),size=2)+theme(legend.position="none",axis.text.x=element_text(angle=90))+scale_color_manual(values=colr)+geom_text(aes(label=tot),vjust=0.3)+labs(x="State - Less Suicides")
